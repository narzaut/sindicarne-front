		...
		<div className='flex flex-col  titillium-font w-full  items-center justify-center pb-10 mt-6'>
    		...
    		<div className='text-center mx-auto pb-1 text-green  text-shadow'>
					...
					<DatePicker onBlur={() =>
						...
						key={'asd3'} dateFormat="dd/MM/yyyy" selected={startDate.date} className='bg-blue-900 text-gray-100 text-shadow rounded w-52 py-1 text-center' locale="es"  onChange={(date) => {setStartDate({...startDate, date:date})}}
					/>
					{startDate.date.getFullYear() > new Date().getFullYear() - 17  ? <p className='text-shadow text-left pt-1  text-xs text-red-500 font-bold'>* Debe ser mayor de 17 años</p> : ''}
				</div>
    		...
    		<TextInput type={'tel'} key={'asd5'} value='telPostulante' errors={errors.telPostulante}  placeholder='Telefono / Celular' register={register} required={true}/>
